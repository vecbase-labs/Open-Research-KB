...
function createServer(): McpServer {
  ...
  server.registerTool(
    'search_math_problem',
    {
      title: 'Search Math Problem',
      description: 'Two-stage workflow for natural-language math problems: first ask the agent to rewrite into search queries, then search those generated queries for direct evidence.',
      inputSchema: z.object({
        db_name: z.string().nullable().default(null).describe('Knowledge-base name to search. Required when more than one DB exists.'),
        problem: z.string().describe('Original natural-language math problem from the user.'),
        suggested_queries: z.array(z.string()).optional().describe('3-5 English/math queries generated by the agent using its LLM capability, mixing surface-topic terms with inferred mathematical objects and technical tools. Omit on the first call.'),
        ...
        include_theorems: z.boolean().default(true),
        theorem_top_k: z.number().int().min(1).max(50).optional().describe('Per-query theorem/result search limit when include_theorems=true.'),
    ...
    async input => jsonText(await searchMathProblem(input)),
  );
  ...
  server.registerTool(
    'build_technical_index',
    {
      ...
      description: 'Pure-code extractor for theorem-like technical results from existing pages/chunks. Defaults to dry-run preview and does not mutate the knowledge base unless dry_run=false and write=true are both supplied.',
      inputSchema: z.object({
        db_name: z.string().nullable().default(null).describe('Knowledge-base name. Required when more than one DB exists.'),
  ...
  server.registerTool(
    'search_technical_results',
    {
      ...
      description: 'Read-only search over theorem-like extracted technical results. Dynamically extracts from existing pages/chunks and ranks result statements; does not mutate the knowledge base.',
      inputSchema: z.object({
        db_name: z.string().nullable().default(null).describe('Knowledge-base name. Required when more than one DB exists.'),
        query: z.string().describe('Search query for theorem-like statements, assumptions, conclusions, section titles, formula refs, and nearby technical terms.'),
