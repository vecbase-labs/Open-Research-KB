...
function createServer(): McpServer {
  ...
  server.registerTool(
    'create_db',
    {
      ...
      inputSchema: z.object({
        ...
        ocr: z.enum(['never', 'auto', 'required']).default('never'),
  ...
  server.registerTool(
    'create_document',
    {
      ...
      description: 'Add one new searchable local PDF to the knowledge base. Text PDFs are accepted; scanned/image-only PDFs without extractable text are rejected and not indexed unless OCR is explicitly enabled.',
      inputSchema: z.object({
        ...
        ocr: z.enum(['never', 'auto', 'required']).default('never').describe('OCR policy for pages with no extractable text. Default never rejects image-only PDFs instead of silently OCRing; auto/required must be explicitly requested.'),
  ...
  server.registerTool(
    ...
    {
      title: 'Search',
      description: 'Search PDF chunks for direct evidence. Defaults to DuckDB-backed BM25 text search.',
      inputSchema: z.object({
        ...
        mode: z.enum(['hybrid', 'text', 'overlap', 'semantic', 'vector']).default('text').describe('Use text for DuckDB BM25, hybrid for BM25 plus exploratory term overlap, overlap for exploratory term overlap only, semantic for LanceDB side-index recall when populated. vector is a deprecated alias for semantic.'),
