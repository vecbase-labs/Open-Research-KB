...
PDF knowledge-base MCP server for Codex agents.
...
6. `list_documents`
7. `search`
8. `search_math_problem`
9. `check_reasonable`
10. `build_technical_index`
11. `search_technical_results`
12. `get_chunk`
13. `get_page_text`
14. `get_page_image`
...
```text
data/index/      DuckDB knowledge-base index
...
Run the optional smoke test with a local sample PDF:

```bash
KB_MCP_SMOKE_PDF=/absolute/path/to/sample.pdf \
npm run smoke:pdf
```

Add one PDF from the command line:

```bash
npm run create-document -- '{"pdf_path":"/absolute/path/book.pdf","doc_id":"optional-stable-id","title":"Optional Title","authors":["Author"],"tags":["book"],"ocr":"auto"}'
```
...
```json
{
  "query": "monge dimartingale",
  "mode": "hybrid",
  "strict": true,
  "results": [],
  "partial_results": [
    {
      "chunk_id": "optimal-transport-methods-economics:p28:c0",
      "matched_terms": ["monge"]
    }
  ],
  "answerability": {
...
```json
{
  ...
  "suggested_queries": [
    ...
    "optimal transport assignment problem matching workers firms surplus",
    "submodularity supermodularity increasing differences optimal matching theorem"
  ],
  "top_k": 3,
  "filters": {
    "doc_id": "optimal-transport-methods-economics"
  },
  "context_window": 1,
  "include_text": false,
